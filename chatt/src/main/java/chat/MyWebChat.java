...
@ServerEndpoint(value = "/chat", encoders = {
    ...
    CommandPojoDecoder.class, ListCommandPojoDecoder.class
})
public class MyWebChat {
  /**
   * Logger for this class
   */
  private static final Logger logger = LoggerFactory.getLogger(MyWebChat.class);
  private static final String USERNAME_SESSION_ATTRIBUTE = "username";

  private static Set<Session> peers = SetUtils.synchronizedSet(new HashSet<Session>());
  ...
  public String onMessage(Session session, CommandPojo command) {
    ...
    switch (command.getCommand()) {
      ...
      case LOGOUT:
        sender = command.getArgumentValue();
        command.setSender("SYSTEM");
        if (StringUtils.isNotBlank(sender) && hasSuchSender(session, sender)) {
          setSender(session, sender);
          sendMessage(session, command);
      ...
      case LOAD_HISTORY:
        ...
        List<CommandPojo> commands = getCommandService().getCommandsFilteredByDate(fromDate);
        try {
  ...
  private boolean hasSuchSender(Session session, String username) {
    return StringUtils.equals((CharSequence) session.getUserProperties().get(USERNAME_SESSION_ATTRIBUTE), username);
  }
  ...
  private String getSender(Session session) {
    String username = (String) session.getUserProperties().get(USERNAME_SESSION_ATTRIBUTE);
    return username;
  }
  ...
  private String sendMessage(Session user, CommandPojo message) {
    for (Session peer : peers) {
      if (peer.isOpen()) {
        ...
        if (peer.equals(user) && isNotLoginAndLogoutCommand(message.getCommand())) {
    ...
    if (user instanceof TyrusSession) {
      message.setIpAddr(((TyrusSession) user).getRemoteAddr());
    }
  ...
  private boolean isNotLoginAndLogoutCommand(Command command) {
    return !Command.LOGIN.equals(command) && !Command.LOGOUT.equals(command);
  }

  private void saveCommandInSeparateThread(final CommandPojo command) {
    if (logger.isInfoEnabled()) {
      logger.info("saveCommandInSeparateThread(CommandPojo) - CommandPojo command={}", command);
    }
  ...
  private CommandService getCommandService() {
    if (commandService == null) {
      commandService = ApplicationContextProvider.getApplicationContext().getBean(CommandService.class);
    }
