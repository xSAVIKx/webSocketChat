...
public class ConsoleChatClient {
  ...
  private final static String MESSAGE_FORMAT = "%s|%s|%s|%s : %s\n";
  ...
  private final static DateTime TODAY = DateTime.now().secondOfDay().setCopy(0);

  public static void main(String[] args) throws URISyntaxException, IOException, EncodeException, InterruptedException {
    ...
    try (BufferedReader reader = new BufferedReader(new InputStreamReader(System.in, "UTF-8"))) {
      ...
      while (!EXIT_COMMAND.equalsIgnoreCase((input = reader.readLine()))) {
        if (HELP_COMMAND.equalsIgnoreCase(input)) {
          ...
          System.err.println("History saved successfully. Check file \"history.txt\"");
        } else {
  ...
  private static void saveHistoryToFile(List<CommandPojo> history) {
    ...
    try (BufferedWriter writer = new BufferedWriter(new FileWriter(historyFile))) {
      for (CommandPojo command : history) {
        writer.write(String.format(MESSAGE_FORMAT, command.getTimestamp(), command.getIpAddr(), command.getSender(),
            command.getCommand().name(), command.getArgumentValue()));
      }
    } catch (IOException e) {
  ...
  private static CommandPojo getSendMessageCommand(String message, String username) {
